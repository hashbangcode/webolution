...
class PagePopulationDecoratorTestBase extends \PHPUnit_Framework_TestCase
{
    ...
    public function setup()
    {
        ...
        $text1 = $this->prophet->prophesize('Hashbangcode\Wevolution\Type\Page\Page');
        $text1->getBody()->willReturn();
        $text1->getStyles()->willReturn();

        $text2 = $this->prophet->prophesize('Hashbangcode\Wevolution\Type\Page\Page');
        $text2->getBody()->willReturn();
        $text2->getStyles()->willReturn();

        $text3 = $this->prophet->prophesize('Hashbangcode\Wevolution\Type\Page\Page');
        $text3->getBody()->willReturn();
        $text3->getStyles()->willReturn();

        $textPopulation = $this->prophet->prophesize('Hashbangcode\Wevolution\Evolution\Population\PagePopulation');

        $textIndividual1 = $this->prophet
            ->prophesize('Hashbangcode\Wevolution\Evolution\Individual\PageIndividual');
